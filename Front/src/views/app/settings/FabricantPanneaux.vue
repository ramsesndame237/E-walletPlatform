  ...
  <v-container fluid grid-list-xl>
    <v-dialog v-model="dialog" width="800px">
      <v-card elevation="15" class="special-scroll">
        <v-card-title>Fabriquants de panneaus</v-card-title>
        <v-container>
          <v-form class="mx-5">
            <v-row>
              <v-col cols="12" md="12">
          ...
          <v-sheet class="v-sheet--offset mx-auto py-3 px-3" color="amber darken-4" elevation="10" max-width="calc(100% - 32px)">
            <v-card-text class="pt-0">
              <div class="title font-weight-thin mb-0 white--text">
                Fabriquants de panneau enrégistrés
...
export default {
  data () {
    return {
      ...
      fabriquants: [],
      headers: [
        { text: 'Fabriquant panneau', value: 'label' },
  ...
  methods: {
    ...
    saveToLocal () {
      ...
      window.localStorage.setItem('jules-app-fabriquant-panneau', data)
    },
    save () {
  ...
  mounted () {
    ...
    if (window.localStorage.getItem('jules-app-fabriquant-panneau') !== null && window.localStorage.getItem('jules-app-fabriquant-panneau').length > 0) { this.fabriquants = JSON.parse(window.localStorage.getItem('jules-app-fabriquant-panneau')) }
  },
}
</script>
