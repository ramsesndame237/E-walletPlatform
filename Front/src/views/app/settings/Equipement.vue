  ...
  <v-container fluid grid-list-xl>
    <v-dialog v-model="dialog" width="800px">
      <v-card elevation="15" class="special-scroll">
        <v-card-title>Nouvelle equipement</v-card-title>
        <v-container>
          <v-form class="mx-5">
            <v-row>
              <v-col cols="12" md="12">
                <v-text-field v-model="editedEquipement.label" label="Solution" required></v-text-field>
              </v-col>
              <v-col cols="12" md="12">
                <v-text-field v-model="editedEquipement.description" label="Type de puissance"></v-text-field>
              </v-col>
            </v-row>
          </v-form>
          <v-row v-show="editedEquipement.label">
            <v-col cols="12">
    ...
    <v-layout wrap class="px-0">
      <v-flex sm12 xs12 md12 lg12>
        <v-card tile class="mx-auto mt-8">
          ...
          <v-card-text class="pt-0" v-if="equipements">
            <v-data-table :headers="headers" :items="equipements" :items-per-page="10" class="mb-3">
...
export default {
  data () {
    return {
      ...
      equipements: [],
      headers: [
        { text: 'Equipement', value: 'label' },
        { text: 'Type de puissance', value: 'description' },
  ...
  methods: {
    editItem (item) {
      this.editedIndex = this.equipements.indexOf(item)
      this.editedEquipement = Object.assign({}, item)
      this.dialog = true
    },
    deleteItem (item) {
      const index = this.equipements.indexOf(item)
    ...
    close () {
      this.dialog = false
      setTimeout(() => {
        this.editedEquipement = Object.assign({}, this.newEquipement)
        this.editedIndex = -1
      }, 300)
    },
    saveToLocal () {
      let data = JSON.stringify(this.equipements)
      window.localStorage.setItem('jules-app-equipement', data)
    },
    save () {
      if (this.editedIndex > -1) {
        Object.assign(this.equipements[this.editedIndex], this.editedEquipement)
      } else {
        this.equipements.push({
          id: Math.floor(new Date () * Math.random() * Math.floor(9)),
          label: this.editedEquipement.label,
          description: this.editedEquipement.description
        })
      }
      this.saveToLocal()
      this.close()
