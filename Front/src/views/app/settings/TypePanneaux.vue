  ...
  <v-container fluid grid-list-xl>
    <v-dialog v-model="dialog" width="800px">
      <v-card elevation="15" class="special-scroll">
        <v-card-title>Types de panneau</v-card-title>
        <v-container>
          <v-form class="mx-5">
            <v-row>
              <v-col cols="12" md="12">
...
export default {
  ...
  methods: {
    ...
    saveToLocal () {
      ...
      window.localStorage.setItem('jules-app-type-panneau', data)
    },
    save () {
  ...
  mounted () {
    ...
    if (window.localStorage.getItem('jules-app-fabriquant-panneau') !== null && window.localStorage.getItem('jules-app-fabriquant-panneau').length > 0) {
      let fabriquant = JSON.parse(window.localStorage.getItem('jules-app-fabriquant-panneau'))
      fabriquant.forEach(fab => {
        this.fabriquants.push(fab.label)
      })
    }
    if (window.localStorage.getItem('jules-app-type-panneau') !== null && window.localStorage.getItem('jules-app-type-panneau').length > 0) { this.types = JSON.parse(window.localStorage.getItem('jules-app-type-panneau')) }
  },
}
</script>
