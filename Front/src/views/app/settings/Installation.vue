  ...
  <v-container fluid grid-list-xl>
    <v-dialog v-model="dialog" width="800px">
      <v-card elevation="15" class="special-scroll">
        <v-card-title>Nouvelle installation</v-card-title>
        <v-container>
          <v-form class="mx-5">
            <v-row>
              <v-col cols="12" md="12">
                <v-text-field v-model="editedInstallation.label" label="Solution" required></v-text-field>
              </v-col>
              <v-col cols="12" md="12">
                <v-text-field v-model="editedInstallation.description" label="Type d'installation à concevoir"></v-text-field>
              </v-col>
            </v-row>
          </v-form>
          <v-row v-show="editedInstallation.label">
            <v-col cols="12">
    ...
    <v-layout wrap class="px-0">
      <v-flex sm12 xs12 md12 lg12>
        <v-card tile class="mx-auto mt-8">
          ...
          <v-card-text class="pt-0" v-if="installations">
            <v-data-table :headers="headers" :items="installations" :items-per-page="10" class="mb-3">
...
export default {
  data () {
    return {
      ...
      installations: [],
      headers: [
        { text: 'Type Installation', value: 'label' },
        { text: 'Type d\'installation à concevoir', value: 'description' },
  ...
  methods: {
    editItem (item) {
      ...
      this.editedInstallation = Object.assign({}, item)
      this.dialog = true
    },
    deleteItem (item) {
      const index = this.installations.indexOf(item)
    ...
    close () {
      this.dialog = false
      setTimeout(() => {
        this.editedInstallation = Object.assign({}, this.newInstallation)
        this.editedIndex = -1
      }, 300)
    },
    saveToLocal () {
      let data = JSON.stringify(this.installations)
      window.localStorage.setItem('jules-app-installation', data)
    },
    save () {
      if (this.editedIndex > -1) {
        Object.assign(this.installations[this.editedIndex], this.editedInstallation)
      } else {
        this.installations.push({
          id: Math.floor(new Date () * Math.random() * Math.floor(9)),
          label: this.editedInstallation.label,
          description: this.editedInstallation.description
        })
      }
      this.saveToLocal()
      this.close()
  ...
  mounted () {
    ...
    if (window.localStorage.getItem('jules-app-installation') !== null && window.localStorage.getItem('jules-app-installation').length > 0) { this.installations = JSON.parse(window.localStorage.getItem('jules-app-installation')) }
  },
}
</script>
