  ...
  <v-container fluid grid-list-xl>
    <v-dialog v-model="dialog" width="800px">
      <v-card elevation="15" class="special-scroll">
        <v-card-title>Fabriquants de regulateurs</v-card-title>
        <v-container>
          <v-form class="mx-5">
            <v-row>
              <v-col cols="12" md="12">
                <v-text-field v-model="editedFabriquant.label" label="fabriquant" required></v-text-field>
              </v-col>
              <!-- <v-col cols="12" md="12">
                <v-text-field v-model="editedFabriquant.description" label="Description"></v-text-field>
              </v-col> -->
            </v-row>
          </v-form>
          <v-row v-show="editedFabriquant.label">
            <v-col cols="12">
          ...
          <v-sheet class="v-sheet--offset mx-auto py-3 px-3" color="amber darken-4" elevation="10" max-width="calc(100% - 32px)">
            <v-card-text class="pt-0">
              <div class="title font-weight-thin mb-0 white--text">
                Fabriquants de regulateur enrégistrés
...
export default {
  data () {
    return {
      ...
      fabriquants: [],
      headers: [
        { text: 'Fabriquant regulateur', value: 'label' },
  ...
  methods: {
    editItem (item) {
      this.editedIndex = this.fabriquants.indexOf(item)
      this.editedFabriquant = Object.assign({}, item)
      this.dialog = true
    },
    deleteItem (item) {
      const index = this.fabriquants.indexOf(item)
    ...
    close () {
      this.dialog = false
      setTimeout(() => {
        this.editedFabriquant = Object.assign({}, this.newfabriquant)
        this.editedIndex = -1
      }, 300)
    },
    saveToLocal () {
      let data = JSON.stringify(this.fabriquants)
      window.localStorage.setItem('jules-app-fabriquant-regulateur', data)
    },
    save () {
      if (this.editedIndex > -1) {
        Object.assign(this.fabriquants[this.editedIndex], this.editedFabriquant)
      } else {
        this.fabriquants.push({
          id: Math.floor(new Date () * Math.random() * Math.floor(9)),
          label: this.editedFabriquant.label,
          description: this.editedFabriquant.description
        })
      }
      this.saveToLocal()
      this.close()
  ...
  mounted () {
    ...
    if (window.localStorage.getItem('jules-app-fabriquant-regulateur') !== null && window.localStorage.getItem('jules-app-fabriquant-regulateur').length > 0) { this.fabriquants = JSON.parse(window.localStorage.getItem('jules-app-fabriquant-regulateur')) }
  },
}
</script>
