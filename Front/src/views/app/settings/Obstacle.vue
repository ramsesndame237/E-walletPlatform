  ...
  <v-container fluid grid-list-xl>
    <v-dialog v-model="dialog" width="800px">
      <v-card elevation="15" class="special-scroll">
        <v-card-title>Nouvelle obstacle</v-card-title>
        <v-container>
          <v-form class="mx-5">
            <v-row>
              <v-col cols="12" md="12">
                <v-text-field v-model="editedObstacle.label" label="Solution" required></v-text-field>
              </v-col>
              <v-col cols="12" md="12">
                <v-text-field v-model="editedObstacle.description" label="Description"></v-text-field>
              </v-col>
            </v-row>
          </v-form>
          <v-row v-show="editedObstacle.label">
            <v-col cols="12">
    ...
    <v-layout wrap class="px-0">
      <v-flex sm12 xs12 md12 lg12>
        <v-card tile class="mx-auto mt-8">
          ...
          <v-card-text class="pt-0" v-if="obstacles">
            <v-data-table :headers="headers" :items="obstacles" :items-per-page="10" class="mb-3">
...
export default {
  data () {
    return {
      ...
      obstacles: [],
      headers: [
        { text: 'Type obstacle', value: 'label' },
  ...
  methods: {
    editItem (item) {
      this.editedIndex = this.obstacles.indexOf(item)
    ...
    close () {
      this.dialog = false
      setTimeout(() => {
        this.editedObstacle = Object.assign({}, this.newObstacle)
        this.editedIndex = -1
      }, 300)
    },
    saveToLocal () {
      let data = JSON.stringify(this.obstacles)
      window.localStorage.setItem('jules-app-obstacle', data)
    },
    save () {
      if (this.editedIndex > -1) {
        Object.assign(this.obstacles[this.editedIndex], this.editedObstacle)
      } else {
        this.obstacles.push({
          id: Math.floor(new Date () * Math.random() * Math.floor(9)),
          label: this.editedObstacle.label,
          description: this.editedObstacle.description
        })
      }
      this.saveToLocal()
      this.close()
